...
- **Forward-only** — a program with `def` / `@cold` / `async` / matrices returns `result.ok: false`
  with a message naming the inexpressible construct. **This is expected**, not a failure of the
  tool: those constructs are forward-only by design (spec §10).
